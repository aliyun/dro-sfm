...
class VideoRandomDataset(Dataset):
    ...
    def _get_context_file_paths_random(self, filename, filelist):
        # fidx = get_idx(filename)
        fidx = filelist.index(filename)
        idxs_back = list(np.arange(-self.backward_context * self.strides, 0, 1))
        
        idx_forw= list(np.arange(0, self.forward_context * self.strides, 1) + 1)
        
        idxs_back = np.random.choice(idxs_back, self.backward_context).tolist()
            
        idx_forw = np.random.choice(idx_forw, self.forward_context).tolist()
        
        idxs = idxs_back + idx_forw
        
        return [filelist[i + fidx] for i in idxs]
